			...
			success: function(str) {
				...
				$('#selectAll').click(function() {
					...
					if($('#selectAll')[0].checked) {
						//一个个更新某个店铺总价
						for(var a = 0; a < $('tbody span').length; a++) { //获取店铺数量
							for(var b = 0; b < storegoodnum[a]; b++) { //获取店铺商品数量
								//获取商品下标
								var d = 0;
								for(var c = 0; c < a; c++) {
									d += storegoodnum[c];
				...
				//2.点击tbody的选中按钮
				$('tbody').on('click', 'input', function() { //事件委托
					var storenum = $(this).attr('class'); //找到所有的input商品列
					if(storenum != 'updatenum') { //点击的是商品勾选input
						//获取某类商品的总价格
	...
	function rendercarshop(str) {
		...
		if(arr.length != 0) {
			...
			var s = arr.map(function(item, index) {
				if(item.storename != currentstorename) { //如果item店铺名与当前店铺名不一样，新建一个分类
					i++;
