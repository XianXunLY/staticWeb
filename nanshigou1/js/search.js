jQuery($ => {
	...
	function searchrender(str) { //查询页面goods数据渲染函数，返回拼接好的字符串
		var arr = JSON.parse(str);
		pagemax = parseInt(arr.pagemax); //更新最大页数
		var s = arr.datalist.map(function(item) { //map方法返回的是个数组，用join方法把它变成可以用来渲染的字符
	...
	function commendrender(str) { //查询页面commendgoods数据渲染函数，返回拼接好的字符串
		var arr = JSON.parse(str);
		var s = arr.map(function(item) { //map方法返回的是个数组，用join方法把它变成可以用来渲染的字符
	...
	$('#searchContent nav .paixu #pricerange #rangesearch').click(function() {
		//重新加载数据，更新查询条件
		if($('#searchContent nav .paixu #max').val().trim()) { //有最大范围就可以查询数据
	...
	$('#goodspageul').on('click', 'li', function() {
		...
		if($(this).index() == 0) {
			if(page > 1) { //如果当前页数大于1的话就可以加载首页内容
				page = 1;
				search();
			}
		} else if($(this).index() == 1) { //确认点击的是上一页按钮
			if(page > 1) { //如果当前页数大于1的话就可以加载上一页内容
				page--;
				search();
			}
		} else if($(this).index() > 1) { //点击的不是首页和上一页
			//找到下一页按钮所在的索引
			var pagelilast2 = $(this).parent().children('li:last').prev().index();
			if($(this).index() == pagelilast2) { //点击的是下一页
				//如果页数小于pagemax就可以加载下一页
				if(page < pagemax) {
					page++;
					search();
				}
			} else if($(this).index() == pagelilast2 + 1) { //点击的是尾页
				if(page < pagemax) { //如果当前页数小于pagemax的话就可以加载尾页内容
					page = pagemax;
					search();
				}
			} else { //点击的是有页码的li
	...
	$('#goodsrender').on('click', '.goodscontentimg', function() { //时间委托，处理动态创建的li点击事件
		var goodid = $(this).closest('li').attr('goodid'); //获取父辈li渲染出来时自定义属性id
		location.href = 'detail.html?' + 'goodsId=' + goodid; //传商品goodid跳转页面
	});
	$('#goodsrender').on('click', 'a', function() { //时间委托，处理动态创建的li点击事件
		var goodid = $(this).closest('li').attr('goodid');
		location.href = 'detail.html?' + 'goodsId=' + goodid;
	});

	//点击加入购物车按钮	
	$('#goodsrender').on('click', 'input', function() { //时间委托，处理动态创建的加入购物车按钮点击事件
